...
local function get_lsp_severity_count(severity)
    -- @type integer
    local severity_count = vim.tbl_count(vim.diagnostic.get(0, { severity = severity }))
...
local function lsp()
    ...
    errors = get_string_format_lsp_severity('E', errors_count)
    warnings = get_string_format_lsp_severity('W', warnings_count)
    hints = get_string_format_lsp_severity('H', hints_count)
